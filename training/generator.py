...
class DataGenerator(Sequence):
    def __init__(
        ...
        self.images = glob.glob(data_path + "/*.tif")
        ...
        tfdataset = tfdataset.shuffle(1024) # this might be a performace problem
        ...
        if self.validation_split:
            ...
            self.tfdataset_validation = self.tfdataset_validation.prefetch(tf.data.experimental.AUTOTUNE)
        ...
        self.tfdataset_train = self.tfdataset_train.prefetch(tf.data.experimental.AUTOTUNE)
        self.tfdataset_train = self.tfdataset_train.repeat()

    def prepare_dataset(self):
        dbg("Preparing dataset")
        with tf.compat.v1.python_io.TFRecordWriter(self.tf_record_file_name) as writer:
            for i, entry in enumerate(tqdm(self.images, total=self.dataset_size)):
                # load and preprocess
                image_data = cv2.imread(entry)
                image_data = preprocess_input(image_data)

                # cut path and extension to get id
                image_id = os.path.basename(entry).replace(".tif", "")
    ...
    def _parse_function(self, serialized):
        ...
        image_data = tf.io.decode_raw(parsed_example["image_data"], tf.float32)
        # We can change it to uint8 probably, but i doubt it will change performance on 64 bit cpu
